...
func GetName() string {
	return "Limit Wheel-based Speed to 60 km/h (TUX)"
}
...
func Run(data flowroutines.FlowData) (flowroutines.FlowData, bool) {
	...
	manipulatedKmh := float64(60 + rand.NormFloat64()*0.0325)
	...
	logdata.Identifier = "Speed Limiter (TUX)"
	logdata.Timestamp = time.Now().UnixNano()
	logdata.Msg = "Manipulated Frame with PGN 65096 by capping the speed at 80 kmh (TUX)"
